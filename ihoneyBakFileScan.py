...
class BakScan(Thread):
    ...
    def _auth(self, url):
        try:
            r = requests.head(url=url, headers=self.headers, timeout=self.timeout, allow_redirects=False, stream=True, verify=False)
            # 验证压缩包文件头10个字节，rar固定头16进制为:526172211a0700cf9073  zip为:504b0304140000000800
            content = b2a_hex(r.raw.read(10)).decode()
            if r.status_code == 200:
                rarsize = ("%.1f" % (int(r.headers.get('Content-Length')) / 1024))
                #if content.startswith('526172') or content.startswith('504b03'):
                logging.warning('\033[31m[*] {}  size:{}kb'.format(url, rarsize))
                with open('success.txt', 'a') as f:
                    try:
                        f.write('[*] {}  size:{}kb'.format(url, rarsize) + '\n')
                    except:
                        pass
            else:  # 如果你只想看扫描成功的备份地址就注释这行和下一行
                logging.warning('\033[36m[ ] {}'.format(url))
...
def dispatcher(url_file=None, url=None, max_thread=1, dic=None):
    ...
    for u in urllist:
        ...
        current_info_dic.extend([u.replace('http://','').replace('https://','').replace('.', '_') + '.rar', u.replace('http://','').replace('https://','').replace('.', '_') + '.zip', u.replace('http://','').replace('https://','').replace('.', '_') + '.tar.gz'])
        current_info_dic.extend([u.replace('http://','').replace('https://','') + '.rar', u.replace('http://','').replace('https://','') + '.zip', u.replace('http://','').replace('https://','') + '.tar.gz'])
       
        if tld.parse_tld(u)[1]:
            current_info_dic.extend([tld.parse_tld(u)[1] + ".rar",tld.parse_tld(u)[1] + ".zip",tld.parse_tld(u)[1] + ".tar.gz"])            
            current_info_dic.extend([u.replace('http://','').replace('https://','').replace('.', '') + '.rar', u.replace('http://','').replace('https://','').replace('.', '') + '.zip', u.replace('http://','').replace('https://','').replace('.', '') + '.tar.gz'])            
            current_info_dic.extend([u.split('.', 1)[-1] + '.rar', u.split('.', 1)[-1] + '.zip', u.split('.', 1)[-1] + '.tar.gz'])
            current_info_dic.extend([wwwhost + '.rar', wwwhost + '.zip', wwwhost + '.tar.gz'])
